  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dummyEmail = isPhone 
        ? `${cleanPhone}@student.cbcexaminationanalyser.ke`
        : `${sanitizedInput.toLowerCase().replace(/[^0-9a-z]/g, '')}@student.cbcexaminationanalyser.ke`;
      ...
      // 3. Fallback: Check students table directly for ADM and Name verification (Legacy/Initial Login)
      ...
      if (studentByAdm) {
        ...
        if (names.includes(inputPassword) || password === 'password123') {
          ...
          if (existingProfile) {
            ...
            // If they have a profile but no Auth, we should ideally sign them up here too
            // But for simplicity, let's just navigate and let the dashboard handle it or they'll fail RLS
            ...
            await supabase.from('profiles').insert({
              id: profileId,
              student_id: studentByAdm.id,
              school_id: studentByAdm.school_id,
              name: studentByAdm.name,
              role: 'student',
              password: password,
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <Link to="/" className="flex flex-col items-center gap-2 mb-10 group">
          ...
          <div className="text-center mt-4">
            <span className="text-4xl font-black text-white tracking-tighter uppercase italic">CBC EXAMINATION ANALYSER <span className="text-black">STUDENTS</span></span>
          </div>
        </Link>
        ...
        <p className="mt-10 text-center text-[10px] font-black text-white uppercase tracking-[0.5em]">
          &copy; 2026 CBC EXAMINATION ANALYSER Student Hub
        </p>
      </div>
    </div>
  );
